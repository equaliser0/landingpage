...
<template>
  <div class="overflow-x-auto bg-base-100 rounded px-4 py-8">
    <div class="flex flex-col justify-center items-center">
      <div class="">
        <button class="btn" onclick="add_user.showModal()">+ Hinzufügen</button>
        <dialog id="add_user" class="modal">
          <div class="modal-box">
            <AdminUserEditor :user="undefined"/>
          </div>
    ...
    <table class="table table-xs">
      ...
      <tbody>
        ...
        <!-- Group List -->
        <tr v-for="user in userStore.user" class="hover:bg-base-200">
          <th>
            <label>
              <input type="checkbox" class="checkbox" />
            </label>
            ...
            <div class="flex items-center space-x-3">
              <div class="avatar">
                <div class="mask mask-squircle w-12 h-12">
                  <img :src="(user.image) ? user.image : 'https://www.bsn.eu/wp-content/uploads/2016/12/user-icon-image-placeholder-300-grey.jpg'" />
                </div>
              </div>
            </div>
          </th>
          ...
          <th> {{ formatReadableDate(user.created) }}</th>
          <th> {{ formatReadableDate(user.updated) }}</th>
